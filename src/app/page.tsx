        ...
        <div className="grid grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    {catTasks.slice(0, 3).map(task => (
                      <div key={task.id} className="flex items-center gap-3">
                        ...
                        <span className={`text-sm ${task.status === 'done' ? 'line-through text-stone-400' : 'text-stone-700'}`}>
                          {task.title}
                        </span>
                      </div>
                    ))}
                    {catTasks.length > 3 && (
                      <Link href={`/estate/${id}/tasks`} className="text-xs text-stone-400 hover:text-stone-600 ml-7">
          ...
          <div className="space-y-4">
            <div className="bg-white border border-stone-200 rounded-xl p-5">
              ...
              <Link
                href={`/estate/${id}/members`}
                className="mt-4 block text-center text-sm text-stone-500 border border-stone-200 rounded-lg py-2 hover:bg-stone-50 transition-colors"
              >
                + Invite member
              </Link>
            </div>
