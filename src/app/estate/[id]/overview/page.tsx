...
export default async function EstateOverviewPage({ params }) {
  ...
  if (tasks) tasks.forEach(task => {
    const catName = task.categories ? task.categories.name : 'General'
      ...
      <div className='max-w-5xl mx-auto px-6 py-10'>
        ...
        <div className='bg-white border border-stone-200 rounded-xl p-6 mb-6'>
          ...
          <div className='w-full bg-stone-100 rounded-full h-3'>
            <div className='bg-stone-800 h-3 rounded-full' style={{ width: progress + '%' }} />
          </div>
                ...
                <div key={category} className='bg-white border border-stone-200 rounded-xl p-5'>
                  <div className='flex items-center justify-between mb-3'>
                    <div className='flex items-center gap-2'>
                      <span>{catTasks[0].categories ? catTasks[0].categories.icon : '📋'}</span>
                  ...
                  <div className='space-y-2'>
                    ...
                    {catTasks.length > 3 && (
                      <Link href={'/estate/' + id + '/tasks'} className='text-xs text-stone-400 ml-7'>+{catTasks.length - 3} more</Link>
                    )}
                  </div>
                </div>
              )
            ...
            <div className='bg-white border border-stone-200 rounded-xl p-5'>
              ...
              <div className='space-y-3'>
                {members && members.map(member => (
                  <div key={member.id}>
                    <p className='text-sm font-medium text-stone-700'>{member.profiles ? member.profiles.full_name : member.email}</p>
                    <p className='text-xs text-stone-400 capitalize'>{member.role}</p>
                  </div>
                ))}
              </div>
              <Link href={'/estate/' + id + '/members'} className='mt-4 block text-center text-sm text-stone-500 border border-stone-200 rounded-lg py-2 hover:bg-stone-50'>+ Invite member</Link>
            </div>
          </div>
        </div>
      </div>
